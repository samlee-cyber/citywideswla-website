...
document.querySelectorAll("[data-case-filter]").forEach((button) =>
  button.addEventListener("click", () => {
    document
      .querySelectorAll("[data-case-filter]")
      .forEach((b) => b.setAttribute("aria-pressed", String(b === button)));
    let count = 0;
    document.querySelectorAll("[data-case-category]").forEach((card) => {
      card.hidden =
        button.dataset.caseFilter !== "All" &&
        card.dataset.caseCategory !== button.dataset.caseFilter;
      if (!card.hidden) count++;
    });
    document.querySelector("#filter-status").textContent = count
      ? `${count} case studies shown.`
...
const allowedEvents = new Set([
  ...
  "walkthrough_form_submit_success",
...
if (form) {
  track("walkthrough_form_view");
  form.addEventListener("focusin", () => track("walkthrough_form_start"), {
    once: true,
  });
}
if (document.querySelector(".error-summary"))
  track("walkthrough_form_submit_error");
function trackReceipt() {
  const id = document.body.dataset.receipt;
  if (!id || window.cityWideAnalyticsConsent !== true) return;
  try {
    const key = "cw-received-" + id;
    if (!sessionStorage.getItem(key)) {
      track("walkthrough_form_submit_success");
      sessionStorage.setItem(key, "1");
    }
  } catch {
    /* No storage: avoid an undeduplicated conversion. */
  }
}
trackReceipt();
window.addEventListener("citywide:analytics-consent", () => {
  if (form) track("walkthrough_form_view");
  trackReceipt();
});
