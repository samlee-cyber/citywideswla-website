...
const navLink = (p, current) =>
  `<a href="${e(p.slug)}"${p.slug === current ? ' aria-current="page"' : ""}${p.type === "action" ? ' class="button button-small"' : ""}>${e(p.label)}${p.type === "action" ? arrow : ""}</a>`;
...
function footer(registry) {
  return `<footer class="footer"><div class="container"><div class="footer-grid"><div>${brand()}<p>Commercial building maintenance.<br>One accountable management partner.</p></div><div><h2>Your local office</h2><address>${e(b.address.streetAddress)}<br>${e(b.address.addressLocality)}, ${b.address.addressRegion} ${b.address.postalCode}</address><a href="tel:${b.telephone}">${b.phone}</a></div><div><h2>Explore</h2><a href="/#how-we-work">How We Work</a>${navigation(
    ...
    .join(
      "",
    )}</div><div><h2>Let’s talk</h2><a href="${b.contactRoute}">Request a Walkthrough</a><a href="https://gocitywide.com/southwestlosangeles/why-work-at-city-wide/">Careers</a><a href="https://gocitywide.com/contractors/">Contractor opportunities</a></div></div><div class="footer-bottom"><span>© 2026 ${e(b.name)}</span><div><a href="/integrations/">Inquiry API</a><a href="/privacy/">Privacy</a><a href="${b.parent.url}">City Wide national ${arrow}</a></div></div></div></footer><div class="mobile-cta"><a href="tel:${b.telephone}">Call our local team</a><a href="${b.contactRoute}">Request a Walkthrough ${arrow}</a></div>`;
}
export function metrics() {
  return `<section class="container about-proof" aria-label="Regional operational scale"><h2>Regional support, local accountability.</h2><div class="metrics">${b.metrics.map((m) => `<div><strong>${m.value}</strong><span>${e(m.label === "Service Crews" ? "Service provider crews" : m.label)}</span></div>`).join("")}</div><p>Across the Los Angeles region, as provided by local ownership in September 2026. City Wide coordinates service providers through a local management team; these figures do not represent full-time onsite City Wide employees at each facility.</p></section>`;
}
...
export function cards(records, klass = "", registry = pages, level = 3) {
  return `<div class="cards ${klass}">${records
    .filter((p) => publicDestination(p, registry))
    .map(
      (p) =>
        `<a class="card" href="${e(publicDestination(p, registry))}" data-event="service_cta_click"><h${level}>${e(p.label)}</h${level}><p>${e(p.slug === janitorialPath ? "Commercial cleaning with a fractional facility manager, a supporting team and an agreed service program." : p.description)}</p><span class="card-label">${isPublished(p, registry) ? "Explore " + e(p.type === "service" ? "service" : p.type === "resource" ? "guide" : "details") : "Discuss this " + e(p.type === "service" ? "service" : "facility")} ${arrow}</span></a>`,
    )
    .join("")}</div>`;
}
...
function managedServices(registry) {
  ...
  return `<section class="service-group">${cards([janitorial], "managed-highlight", registry, 2)}</section>${serviceGroups
    .map(
      ([title, slugs]) =>
        `<section class="service-group" id="${title.toLowerCase().replaceAll(" ", "-")}"><h2 class="service-group-title">${e(title)}</h2>${cards(
          slugs.map((slug) =>
...
const localAreas = (registry) =>
  `<div class="area-links">${children("/locations/", registry)
    .map(
      (p) =>
        `<a href="${e(publicDestination(p, registry))}"><span>${e(p.label)}<small>${isPublished(p, registry) ? "Explore local services" : "Discuss your facility"}</small></span>${arrow}</a>`,
    )
    .join("")}</div>`;
const buyerQuestions = [
  [
    "What does the manager do?",
    "Your Facility Solutions Manager coordinates contracted providers, oversees quality and follows up on issues. They work as an extension of your team, not a full-time onsite employee.",
  ],
  [
    "What does a managed janitorial contract include?",
    "The agreed janitorial services, a fractional facility manager, a supporting management team and a program with a defined scope, schedule and quality follow-up.",
  ],
  [
    "How are quality issues handled?",
    "Bring concerns to your local manager. The team reviews the issue against the agreed scope, coordinates the next steps with providers and follows up.",
  ],
  [
    "How is pricing determined?",
    "Your proposal reflects the facility’s size, tasks, frequency, access and operating needs. Managed janitorial includes management in the program; additional work is scoped and priced separately.",
  ],
  [
    "Can I request another service independently?",
    "Yes. You do not need a janitorial contract to ask about a repair, maintenance service or improvement project. Tell us what needs attention and we’ll discuss a suitable scope.",
  ],
];
function faqList(questions) {
  return questions
    .map(
      ([question, answer]) =>
        `<details class="faq"><summary>${e(question)} <span aria-hidden="true">+</span></summary><p>${e(answer)}</p></details>`,
    )
    .join("");
}
function caseCards(registry) {
  const studies = children("/case-studies/", registry).filter((p) =>
...
function home(registry) {
  const evidence = children("/case-studies/", registry).filter((p) =>
    isPublished(p, registry),
  );
  return `<section class="hero"><div class="hero-copy"><p class="eyebrow">Commercial Facility Management • Southwest Los Angeles</p><h1>Commercial Building Maintenance.<br><em>Managed for You.</em></h1><p class="hero-description">Less vendor coordination. Less issue follow-up. One local management team bringing your building’s services together around your needs and budget.</p><div class="hero-actions">${button()}<a class="text-link" href="#how-we-work">How We Work ${arrow}</a></div><p class="hero-support">Comparing cleaning providers? <a href="${janitorialPath}">Explore managed janitorial.</a></p></div><div class="hero-visual"><img class="hero-photo" src="/assets/workplace.webp" width="1600" height="1068" fetchpriority="high" alt="Commercial facility with shared workspaces and meeting areas"></div></section>
<section class="section container program" id="how-we-work"><div class="program-intro"><div><p class="eyebrow">How we work</p><h2>Your manager.<br>Your team. Your plan.</h2></div><div><p>${e(programExplanation)}</p><p>Your manager acts as an extension of your team, not a full-time onsite employee. Together, we agree on the scope, schedule and quality expectations.</p></div></div><div class="program-benefits"><div><h3>Coordinate the work</h3><p>One local contact brings service providers, schedules and communication together.</p></div><div><h3>Follow through on issues</h3><p>Your management team oversees quality, coordinates the next steps and follows up.</p></div><div><h3>Plan what comes next</h3><p>We help arrange additional maintenance and projects as your needs change.</p></div></div><p class="program-note">${e(programNote)}</p><p class="quiet-proof">Supporting 150+ facilities across the Los Angeles region.</p></section>
<section class="section paper" id="services"><div class="container">${intro("Managed services", "One partner for your building’s needs.")}<div class="service-overview">${[
    [
      "Interior Care",
      "Janitorial, hard floors, carpets and consumable supplies.",
      "interior-care",
    ],
    [
      "Exterior Care",
      "Grounds, windows, pressure washing and parking areas.",
      "exterior-care",
    ],
    [
      "Repairs and Improvements",
      "Handyman, plumbing, electrical, HVAC and tenant improvements.",
      "repairs-and-improvements",
    ],
  ]
    .map(
      ([title, copy, anchor]) =>
        `<div><h3>${e(title)}</h3><p>${e(copy)}</p><a class="text-link" href="/services/#${anchor}" data-event="service_cta_click">Explore ${e(title.toLowerCase())} ${arrow}</a></div>`,
    )
    .join(
      "",
    )}</div><a class="section-link" href="/services/">View all 13 services ${arrow}</a></div></section>
<section class="section container feature" id="managed-janitorial"><div><p class="eyebrow">Managed janitorial</p><h2>A cleaning contract that takes more off your plate.</h2><p>Get commercial cleaning with a fractional facility manager, a supporting team and an agreed service program. We coordinate providers, oversee quality and address issues.</p><p>Competitive pricing with management built into the program, tailored to your building and schedule. Additional services and projects are scoped and priced separately.</p><div class="hero-actions"><a class="text-link" href="${janitorialPath}" data-event="service_cta_click">Explore managed janitorial ${arrow}</a><a class="text-link" href="${janitorialRequest}" data-event="service_cta_click">Request a janitorial walkthrough ${arrow}</a></div></div><figure><img src="/assets/commercial-cleaning.webp" width="900" height="600" alt="Commercial cleaning staff caring for tables and shared surfaces" loading="lazy"></figure></section>
${evidence.length ? `<section class="section container">${intro("In practice", "Facility problems. Managed solutions.")}${caseCards(registry)}</section>` : ""}
<section class="section paper" id="service-area"><div class="container local-summary"><div><p class="eyebrow">Southwest Los Angeles</p><h2>A local team accountable to you.</h2><p>Based in Long Beach and led by owner Sam Lee, City Wide Southwest Los Angeles helps businesses coordinate facility services and resolve issues through one local management relationship.</p><a class="text-link" href="/about/">Meet our local team ${arrow}</a></div><div><h3>Based in Long Beach</h3><address>${e(b.address.streetAddress)}<br>Long Beach, CA ${b.address.postalCode}</address><h3>Our service area</h3><p>${b.areas
    .map((city) => {
      const p = registry.find((p) => p.type === "location" && p.city === city);
      return p && isPublished(p, registry)
        ? `<a href="${p.slug}">${e(city)}</a>`
        : e(city);
    })
    .join(
      " · ",
    )}</p><a class="text-link" href="/locations/">Discuss your location ${arrow}</a></div></div></section>
<section class="section container buying-questions">${intro("Buying questions", "Know what to expect.")}${faqList(buyerQuestions)}</section>${cta()}`;
}
...
function related(page, registry) {
  const records = page.relationships
    ...
    .filter((p) => p && isPublished(p, registry));
  return records.length
...
function hub(page, registry) {
  ...
  if (page.slug === "/services/") body = managedServices(registry);
    ...
    body = cards(children(page.slug, registry), "resource-grid", registry, 2);
...
export function schema(page, registry) {
  if (!isPublished(page, registry) || page.type === "utility") return null;
...
export function renderPage(
  ...
  const index = isIndexable(page, production, registry) && !error;
  const body =
    bodyOverride ??
    (page.type === "home"
      ? home(registry)
      : !isPublished(page, registry)
        ? construction(page)
        : page.type === "hub"
          ? hub(page, registry)
          : page.type === "about"
