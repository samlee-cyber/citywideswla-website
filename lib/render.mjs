...
function footer(registry) {
  return `<footer class="footer"><div class="container"><div class="footer-grid"><div>${brand()}<p>One local partner.<br>A better-maintained facility.</p></div><div><h2>Your local office</h2><address>${e(b.address.streetAddress)}<br>${e(b.address.addressLocality)}, ${b.address.addressRegion} ${b.address.postalCode}</address><a href="tel:${b.telephone}">${b.phone}</a></div><div><h2>Explore</h2>${navigation(
    registry,
  )
...
export function cards(records, klass = "") {
  return `<div class="cards ${klass}">${records.map((p) => `<a class="card" href="${p.slug}"><div class="card-top">${icon(p.icon)}${arrow}</div><h3>${e(p.label)}</h3><p>${e(p.description)}</p><span class="card-label">${isPublished(p) ? "Explore " + e(p.type === "service" ? "service" : p.type === "resource" ? "guide" : "details") : "In construction"} ${arrow}</span></a>`).join("")}</div>`;
}
...
export function process() {
  return `<ol class="steps"><li><span>01</span><div><h3>Start with your priorities.</h3><p>Tell us about your facility, what’s working, and what needs attention.</p></div></li><li><span>02</span><div><h3>Bring the plan together.</h3><p>Discuss the spaces, access, and service needs. Arrange a walkthrough and tailored scope where appropriate.</p></div></li><li><span>03</span><div><h3>Keep one point of contact.</h3><p>Your local Facility Solutions Manager coordinates providers, communication, and quality follow-up.</p></div></li></ol>`;
}
export function cta(event = "service_cta_click") {
  return `<section class="final-cta" id="contact"><div class="container"><div><p class="eyebrow">Let’s take a look at your facility</p><h2>Start with a conversation.<br>Build a better plan.</h2><p>Tell us about your space and the services you need.</p></div>${button("Request a Walkthrough", b.contactRoute, "", event)}</div></section>`;
}
const atGlance = () =>
  `<aside class="glance"><p class="eyebrow">At a glance</p><dl><dt>Business</dt><dd>${e(b.name)}</dd><dt>Local office</dt><dd>${e(b.address.streetAddress)}, Long Beach, CA ${b.address.postalCode}</dd><dt>Service model</dt><dd>One local contact managing commercial cleaning and facility services.</dd><dt>Initial service areas</dt><dd>${b.areas.join(", ")}</dd><dt>Getting started</dt><dd>Discussion → walkthrough → scope and proposal, as appropriate.</dd></dl><a href="tel:${b.telephone}">${b.phone} ${arrow}</a></aside>`;
...
function home(registry) {
  return `<section class="hero"><div class="hero-copy"><p class="eyebrow">One local partner. One point of contact.</p><h1>Commercial Cleaning &amp; Facility Services in <em>Southwest Los Angeles.</em></h1><p class="hero-description">Your building has a lot of needs. We bring them together. One local partner managing commercial cleaning and a broad range of facility services.</p><div class="hero-actions">${button()}<a class="text-link" href="/services/">Explore Services ${arrow}</a></div></div><div class="hero-visual"><img class="hero-photo" src="/assets/workplace.webp" width="1600" height="1068" fetchpriority="high" alt="Bright commercial workspace with shared seating and meeting areas"><div class="photo-caption"><span>Better spaces.<br>Fewer things on your plate.</span>${arrow}</div></div></section>${metrics()}
<section class="section container feature"><div><p class="eyebrow">The foundation of a well-kept facility</p><h2>Commercial cleaning.<br>Professionally managed.</h2><p>From recurring janitorial to day porter support, build a cleaning plan around your space and the people who use it. Your Facility Solutions Manager coordinates the work and keeps the conversation in one place.</p><ul class="check-list"><li>Recurring janitorial and common-area cleaning</li><li>Day porter support and supply replenishment</li><li>Service coordination and quality follow-up</li></ul><a class="text-link" href="/services/commercial-cleaning/">Explore Commercial Cleaning ${arrow}</a></div><figure><img src="/assets/commercial-cleaning.webp" width="900" height="600" alt="Commercial cleaning staff caring for tables and shared surfaces" loading="lazy"><figcaption>Care for the spaces your people depend on.</figcaption></figure></section>
<section class="section paper" id="services"><div class="container">${intro("The services we manage", "One partner. A full range of facility needs.", "From everyday cleaning to the projects you’ve been meaning to tackle, start with one conversation.")}${cards(children("/services/", registry), "services-grid")}</div></section>
<section class="section container" id="industries">${intro("Industries we serve", "Your environment shapes the plan.", "Discuss your operating hours, access requirements, and the spaces that matter to your business.")}${cards(children("/industries/", registry), "industry-grid")}</section>${customerProof()}
<section class="difference" id="difference"><div class="container difference-grid"><div><p class="eyebrow">Why City Wide</p><h2>One relationship.<br>A whole lot<br><em>less to manage.</em></h2><p>Locally owned and connected to the City Wide network. Your priorities, represented by a partner who stays involved.</p><a class="text-link" href="/about/">Meet your local team ${arrow}</a></div>${process()}</div></section>
<section class="section container">${intro("Case studies", "The work behind the relationship.")}${caseCards(registry)}<a class="section-link" href="/case-studies/">Visit our project library ${arrow}</a></section>
<section class="section paper" id="service-area"><div class="container location-split"><div><p class="eyebrow">Right here in Southwest LA</p><h2>National resources.<br>Local relationships.</h2><p>Based in Long Beach, our local team helps businesses across Southwest Los Angeles coordinate their facility services.</p><a class="text-link" href="/locations/">Explore our service area ${arrow}</a></div>${localAreas(registry)}</div></section>
<section class="section container">${intro("Resources", "Plan your next step.", "We’re building practical guides for facility teams. Preview the topics in development.")}${cards(children("/resources/", registry).slice(0, 3), "resource-grid")}<a class="section-link" href="/resources/">See all planned resources ${arrow}</a></section>${cta()}`;
}
...
function detail(page, registry) {
  return `<div class="container detail-layout"><article>${page.type === "resource" || page.type === "case-study" ? `<p class="byline">By ${e(page.author)} · Published ${e(page.publishedAt)} · Updated ${e(page.updatedAt)}</p>` : ""}${page.content.map((s) => `<section class="content-section"><h2>${e(s.heading)}</h2>${(s.paragraphs || []).map((p) => `<p>${e(p)}</p>`).join("")}${s.items?.length ? `<ul>${s.items.map((i) => `<li>${e(i)}</li>`).join("")}</ul>` : ""}</section>`).join("")}${page.type === "service" ? `<section class="content-section"><h2>Frequently asked questions</h2><details class="faq"><summary>Can cleaning be coordinated with other services? <span aria-hidden="true">+</span></summary><p>Yes. Discuss floor care, supplies, and other facility needs with your local point of contact so they can be considered in the scope.</p></details><details class="faq"><summary>How do we decide on a schedule? <span aria-hidden="true">+</span></summary><p>We discuss your facility’s operating hours, traffic, and access requirements during planning. A schedule is agreed as part of your scope.</p></details></section>` : ""}${page.sources?.length ? `<section class="content-section"><h2>Supporting sources</h2><ul>${page.sources.map((url) => `<li><a href="${e(url)}">${e(url)}</a></li>`).join("")}</ul></section>` : ""}${related(page, registry)}</article>${atGlance()}</div>${cta(page.type === "case-study" ? "case_study_cta_click" : "service_cta_click")}`;
}
function hub(page, registry) {
  let body = "";
  if (page.slug === "/services/")
    body = `<div class="hub-intro"><p>Your Facility Solutions Manager brings service coordination and communication together. Commercial cleaning can be the starting point, with other facility needs managed through the same local relationship.</p></div>${cards(children(page.slug, registry), "services-grid")}`;
