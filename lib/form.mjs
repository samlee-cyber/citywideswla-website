...
export function renderForm({
  ...
  const input = (name, f) => {
    ...
    let element;
    if (f.options)
      element = `<select ${attrs}><option value="">Select ${f.required ? "an option" : "if applicable"}</option>${f.options.map((v) => `<option value="${e(v)}"${values[name] === v ? " selected" : ""}>${e(v)}</option>`).join("")}</select>`;
