...
export const fields = {
  ...
  facility_type: {
    label: "Facility Type",
    max: 100,
    options: [...industryCatalogue.map((s) => s[1]), "Other / not sure yet"],
  },
  service_needed: {
...
export function validateFields(input) {
  ...
  if (!input || typeof input !== "object" || Array.isArray(input))
    return {
      data,
      errors: { _form: "Send an object containing the form fields." },
    };
  for (const [name, f] of Object.entries(fields)) {
    const raw = input[name];
    if (raw !== undefined && typeof raw !== "string") {
      errors[name] = "Use a text value.";
      data[name] = "";
      continue;
    }
    ...
    if (f.required && !value) errors[name] = `${f.label} is required.`;
      ...
      errors[name] = `Use no more than ${f.max} characters.`;
    else if (
      (f.type !== "textarea" && /[\r\n]/.test(value)) ||
      /[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(value)
    )
      errors[name] = "Remove unsupported control characters.";
    else if (
      value &&
      name === "work_email" &&
      !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)
    ...
    else if (value && f.options && !f.options.includes(value))
      errors[name] = "Choose an available option.";
    else if (
      value &&
      name === "facility_size_sqft" &&
      !/^[1-9]\d{0,11}$/.test(value)
    )
      errors[name] = "Enter a positive whole number or leave this blank.";
  }
  return { data, errors };
}
