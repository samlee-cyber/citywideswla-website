...
import { validateRegistry, sitemap, productionReady } from '../lib/publication.mjs';
import { renderForm, renderReceipt } from '../lib/form.mjs';
import { renderPage, notFound } from '../lib/render.mjs';
export async function build({registry=pages,out='dist',production=productionReady()}={}) {
 validateRegistry(registry);
 // Only remove the build output chosen by this command, never the project root.
 const dest=resolve(out);if(dest===process.cwd()||dest==='/')throw new Error('Unsafe output directory');
